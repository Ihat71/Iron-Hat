...
async def get_weight_and_bf_history(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await get_weight_and_bf_history_service(db, current_user)

@router.get("/weight/extra", status_code=status.HTTP_200_OK)
async def get_extra_biometrics(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await get_extra_biometrics_service(db, current_user)

@router.get("/pr-records", status_code=status.HTTP_200_OK)
async def get_pr_tracking(exercises: list[int], db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await get_pr_tracking_service(exercises, db, current_user)

@router.get("/workouts", status_code=status.HTTP_200_OK)
async def get_workout_history(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await get_workout_history_service(db, current_user)

@router.get("/exercises/{exercise_id}", status_code=status.HTTP_200_OK)
async def get_exercise_history(exercise_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await get_exercise_history_service(exercise_id, db, current_user)

@router.get("/summary/{program_id}", status_code=status.HTTP_200_OK)
async def get_summary(program_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await get_tracking_summary(program_id, db, current_user)
