...
async def get_weight_and_bf_history_service(db: AsyncSession, current_user: User):
    return await get_weight_and_bf_history(db, current_user)
...
async def get_extra_biometrics_service(db: AsyncSession, current_user: User):
    bio_history = await get_bio_history(db, current_user)

    return bio_history

async def get_workout_history_service(db: AsyncSession, current_user: User):
    ...
    res = []
    workouts_done = await get_workouts_done(30, db, current_user)
    ...
    for workout in workouts_done:
        ...
        workout_template_result = await db.execute(select(WorkoutTemplate).where(WorkoutTemplate.program_id == workout.program_id, WorkoutTemplate.day_number == workout.day_number))
        workout_template = workout_template_result.scalars().one_or_none()
        template_exercises = await get_user_workout_template_exercises_by_workout_value(db, current_user.id, workout_template.id)
        log_exercises = await get_user_workout_log_exercise_by_workout(db, current_user.id, workout.id)
...
async def get_tracking_summary(program_id:int, db: AsyncSession, current_user: User):
    ...
    """
    res = {}
    res['pr_count'] = await get_pr_count(30, db, current_user)
    res['workouts_done'] = await get_workout_logs_count(program_id, 30, db, current_user)
    weight_per_month = await get_weights_last_month(db, current_user) # returns (weight, date), make sure it has the most recent one as last
    target_consistency = await get_workout_template_target_consistency_per_week(program_id, db, current_user)
