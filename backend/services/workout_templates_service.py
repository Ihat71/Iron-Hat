...
def is_valid(db: Session, user_id: int, workout_id: int, program_id: int) -> bool:
    workout = get_workout_template(db, workout_id, program_id)
    if not workout:
...
def add_workout_template_service(db: Session, data: WorkoutTemplateCreate, program_id: int, user: User):
    ...
    created_template = create_workout_template(db, workout_data)
    ...
    created_exercises = create_workout_template_exercise(db, exercises_data)
...
def get_workout_template_by_params_service(program_id: int, workout_type: str | None, day_number: int | None, db: Session, user: User):
    return get_user_workout_template_by_value(db, workout_type, day_number, program_id, user.id)

def get_all_workout_templates_service(program_id: int, db: Session, user: User):

    workout_templates = get_all_user_workout_templates_by_program_id(db, user.id, program_id)
    workout_read_list = []
    for workout in workout_templates:
        exercises = get_workout_template_exercises_by_workout_id(workout.id, db)
...
def get_workout_template_service(db: Session, program_id: int, workout_id: int, user: User):
    
    return get_workout_template(db, workout_id, program_id)

def update_workout_template_service(db: Session, data: WorkoutTemplateUpdate, program_id: int, workout_id: int, user: User):
    if not is_valid(db, user.id, workout_id, program_id):
        raise ValueError("cant access that")
    
    return update_workout_template(db, workout_id, data)

def delete_workout_template_service(db: Session, program_id: int, workout_id: int, user: User):
    if not is_valid(db, user.id, workout_id, program_id):
        raise ValueError("cant access that")
